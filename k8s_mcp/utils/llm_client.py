...
def handle_tool_calls(client, messages, assistant_message, model, extra_body):
    """
    Handles tool/function calls generated by the assistant and feeds the result back into the chat.

    Args:
        client: OpenAI-compatible chat client.
        messages (List[Dict]): Current chat history including assistant tool call.
        assistant_message (Dict or Object): Assistant response containing tool_calls.
        model (str): The model name used for follow-up completion.
        extra_body (Dict): Additional OpenAI chat parameters.

    Yields:
        str: Streamed assistant response after tool execution.
    """
...
def call_llm(
    ...
    """
    Call LLM (chat model) and optionally allow it to trigger tools via OpenAI-compatible tool calls.

    Args:
        prompt: User input prompt.
        model: Model name; fetched from env if not provided.
        temperature: Sampling temperature.
        thinking: Enable "thinking" (system message injection).
    ...
    Returns:
        Generator yielding streaming tokens or full response string.
    """
